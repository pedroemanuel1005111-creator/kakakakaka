      ...
      <div style={{ background: "var(--bg-card)", padding: 12, borderRadius: 10, marginBottom: 12 }}>
        <div style={{ display: "flex", gap: 10, alignItems: "end", flexWrap: "wrap" }}>
          <div className="form-group" style={{ margin: 0 }}>
            ...
            <select className="form-input" value={category} onChange={(e) => setCategory(e.target.value)} style={{ minWidth: 180 }}>
          ...
          <label className="btn-primary" style={{ cursor: "pointer", display: "inline-block" }}>
            <span>{uploading ? "⏳ Enviando..." : "📤 ENVIAR FOTOS/VÍDEOS"}</span>
        ...
        <p style={{ color: "var(--text-gray)", fontSize: "0.8rem", marginTop: 10 }}>
          Você pode enviar várias fotos e vídeos de uma vez. Os itens da categoria “Galeria do site” aparecem na nova aba acima do contato.
        </p>
      </div>
...
function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    // Fallback de polling a cada 15s (garante sincronia mesmo se SSE falhar)
